...
public:
    ...
    friend SIMDVector3 operator/(const SIMDVector3& vec, float scalar)
    {
        return SIMDVector3(_mm_div_ps(vec.mVec, _mm_set_ps1(scalar)));
    }
    ...
    {
        return SIMDVector3(_mm_dp_ps(mVec, mVec, 0x7f));
    }

    // Length squared of vector
    float LengthSq() const
    {
        return _mm_dp_ps(mVec, mVec, 0x7f).m128_f32[0];
    }

    // Length of vector (as a vector)
    SIMDVector3 Length_v() const
    {
        __m128 dot = _mm_dp_ps(mVec, mVec, 0x7f);
        __m128 rsqrt = _mm_rsqrt_ps(dot);
        return SIMDVector3(_mm_mul_ps(dot, rsqrt));
    }
    ...
    float Length() const
    {
        __m128 dot = _mm_dp_ps(mVec, mVec, 0x7f);
        __m128 rsqrt = _mm_rsqrt_ps(dot);
        return _mm_mul_ps(dot, rsqrt).m128_f32[0];
    }
    ...
    friend SIMDVector3 Cross(const SIMDVector3& a, const SIMDVector3& b)
    {
        ...
        __m128 temp = _mm_mul_ps(tempA, tempB);
        ...
        return SIMDVector3(_mm_sub_ps(temp, tempA));
    }

    // Lerp from A to B by f
    friend SIMDVector3 Lerp(const SIMDVector3& a, const SIMDVector3& b, float f)
    {
        __m128 diff = _mm_sub_ps(b.mVec, a.mVec);
        diff = _mm_mul_ps(diff, _mm_set_ps1(f));
        return SIMDVector3(_mm_add_ps(a.mVec, diff));
    }
...
public:
    ...
    friend SIMDVector4 operator/(const SIMDVector4& vec, float scalar)
    {
        return SIMDVector4(_mm_div_ps(vec.mVec, _mm_set_ps1(scalar)));
    }
    ...
    {
        return SIMDVector4(_mm_dp_ps(mVec, mVec, 0xff));
    }

    // Length squared of vector
    float LengthSq() const
    {
        return _mm_dp_ps(mVec, mVec, 0xff).m128_f32[0];
    }

    // Length of vector (as a vector)
    SIMDVector4 Length_v() const
    {
        __m128 dot = _mm_dp_ps(mVec, mVec, 0xff);
        __m128 rsqrt = _mm_rsqrt_ps(dot);
        return SIMDVector4(_mm_mul_ps(dot, rsqrt));
    }
    ...
    {
        __m128 dot = _mm_dp_ps(vec.mVec, vec.mVec, 0xff);
        __m128 rsqrt = _mm_rsqrt_ps(dot);
        return SIMDVector4(_mm_mul_ps(vec.mVec, rsqrt));
    }
    ...
    friend SIMDVector4 Cross(const SIMDVector4& a, const SIMDVector4& b)
    {
        ...
        tempB = _mm_shuffle_ps(b.mVec, b.mVec, _MM_SHUFFLER(1, 2, 0, 3));
        tempA = _mm_mul_ps(tempA, tempB);
        return SIMDVector4(_mm_sub_ps(temp, tempA));
    }
    ...
    friend SIMDVector4 Lerp(const SIMDVector4& a, const SIMDVector4& b, float f)
    {
        __m128 diff = _mm_sub_ps(b.mVec, a.mVec);
        diff = _mm_mul_ps(diff, _mm_set_ps1(f));
        return SIMDVector4(_mm_add_ps(a.mVec, diff));
    }
...
public:
    ...
    static SIMDMatrix4 CreateOrtho(float width, float height, float nearClip, float farClip)
    {
        float temp[4][4] =
        {
    ...
    static SIMDMatrix4 CreatePerspectiveFOV(float fovY, float width, float height, float nearClip, float farClip)
    {
